...
- CONFIG：显示当前插件配置。v1.1.0 尚未把 Avid Control/S3 上的插件选择与插入作为正式支持功能；请在 UAD Console 中完成插件装载和替换。
...
控制室是独立页面，不占普通通道推子。可控制主监听音量、MUTE、DIM、MONO、Mix/Cue 监听源、Main/ALT1/ALT2 监听组、DIM 深度、TALKBACK 开关和 Talk dB。主音量与 Talk dB 都是连续实时控制。Avid Control 的 Speaker Select 区域还会提供一个 `Main` 入口，它与 Main 监听组执行相同的真实操作。
...
**插件或 CONFIG 项目没有出现**：v1.1.0 尚不正式支持从 Avid Control/S3 选择或插入插件，请在 UAD Console 中完成；已经装载的插件参数仍可通过 EUCON 控制。
