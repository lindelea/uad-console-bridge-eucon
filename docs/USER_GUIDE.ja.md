...
- CONFIG：プラグインの選択・削除、Console に存在するプリセットの呼び出し。回して選択し、`In` で確定します。
...
コントロールルームは通常チャンネルとは別で、チャンネルフェーダーを消費しません。メインモニター音量、MUTE、DIM、MONO、モニターソース、DIM 深度、TALKBACK を操作できます。メイン音量は連続リアルタイム制御で、操作終了まで保留しません。
...
**プラグイン／CONFIG 項目がない：** Console 側でそのスロットや機能に対応すること、設定で CONFIG が有効なことを確認します。一部の CONFIG 表示変更は本アプリ再起動後に反映されます。
